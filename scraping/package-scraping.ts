...
export const startPackageScraping = async (page: Page, pkg: PackageInfo) => {
    ...
    const packageDetails = await page.evaluate(() => {
        ...
        packageDetails.description = packageElement?.querySelector("#pkgOverView p")?.innerHTML.replace(regex, "Travel.com") as string;
        ...
        const cities: DestinationDetailsType[] = []
        ...
        cityElements.forEach((cityElement) => {
            ...
            const cityDescription = document.getElementById("aboutDestPara")
                ?.textContent!.trim()
